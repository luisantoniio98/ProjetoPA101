...
class TabelStringencyIndex(db: SQLiteDatabase) {
    private val db: SQLiteDatabase = db

    fun cria() {
        db.execSQL("CREATE TABLE $NOME_TABELA (${BaseColumns._ID} INTEGER PRIMARY KEY AUTOINCREMENT,$INDEX_RESTRITAMENTO FLOAT NOT NULL, $CAMPO_ID_PAIS INTEGER NOT NULL, FOREIGN KEY($CAMPO_ID_PAIS) REFERENCES ${TabelaPaises.NOME_TABELA})")
    }
    ...
    companion object {
        const val NOME_TABELA = "index_restritamento"
        const val CAMPO_ID_PAIS = "id_pais"
        const val INDEX_RESTRITAMENTO = "index"
    }
}